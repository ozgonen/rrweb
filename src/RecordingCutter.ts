// RecordingCutter.ts - Utility for cutting rrweb recordings into smaller clips
 ...
 * @param events - rrweb events array
 * @param timestamp - Absolute timestamp
 * @returns Relative seconds from start
 */
export function timestampToRelativeTime(events: RRWebEvent[], timestamp: number): number {
 ...
 * @param timeStr - Time string like "1:30", "0:05", "90", etc.
...
export function cutRecording(
  ...
  beforeSeconds: number = 5,
  afterSeconds: number = 5
): RRWebEvent[] {
  ...
  let fullSnapshot: FullSnapshotEvent | null = null;
  ...
  for (const event of events) {
    if (event.type === EventType.FullSnapshot) {
      ...
      if (distance < bestDistance) {
        fullSnapshot = event as FullSnapshotEvent;
 ...
 */
export function findEventsByContent(events: RRWebEvent[], searchTerm: string): RRWebEvent[] {
  const matchingEvents: RRWebEvent[] = [];
 ...
 */
export function analyzeRecording(events: RRWebEvent[]): RecordingStats | null {
  if (!events || !Array.isArray(events) || events.length === 0) {
    return null;
  }
 ...
 */
const pauseAndWait = (replayer: Replayer, relativeTime: number): Promise<void> => {
...
async function createTrueSyntheticSnapshot(events: RRWebEvent[], targetTimestamp: number): Promise<FullSnapshotEvent> {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const domSnapshot = snapshot(iframe.contentDocument, snapshotOptions);
      ...
      const syntheticSnapshot: FullSnapshotEvent = {
        type: 2, // FullSnapshot
        data: {
          node: domSnapshot,
          initialOffset: {
 ...
 */
export async function trimRecording(events: RRWebEvent[], startMs: number, endMs: number): Promise<RRWebEvent[]> {
  if (!events || !Array.isArray(events) || events.length === 0) {
    throw new Error("Invalid events array provided");
  }
  ...
  try {
    ...
    const snapshots = events.filter((e) => e.type === 2) as FullSnapshotEvent[];
    const closeSnapshot = snapshots.find(
      (s) => Math.abs(s.timestamp - startMs) < 1000 // Within 1 second
    );

    let syntheticSnapshot: FullSnapshotEvent;
    ...
    const originalMeta = events.find((e) => e.type === 4) as MetaEvent | undefined;
    ...
    const adjustedSnapshot: FullSnapshotEvent = {
      ...syntheticSnapshot,
      timestamp: 1,
    };
    ...
    for (const event of eventsInRange) {
      const adjustedEvent: RRWebEvent = {
        ...event,
...
function trimRecordingFallback(events: RRWebEvent[], startMs: number, endMs: number): RRWebEvent[] {
  // Find ALL full snapshots and choose the best one
  const fullSnapshots: { snapshot: FullSnapshotEvent; index: number }[] = [];
  for (let i = 0; i < events.length; i++) {
    if (events[i].type === EventType.FullSnapshot) {
      fullSnapshots.push({ snapshot: events[i] as FullSnapshotEvent, index: i });
    }
  }
  ...
  let baseSnapshot: FullSnapshotEvent | null = null;
  ...
  for (let i = baseSnapshotIndex + 1; i < events.length; i++) {
    ...
    if (event.timestamp < startMs) {
      ...
      if (event.type === EventType.IncrementalSnapshot && event.data && event.data.source === 0) {
